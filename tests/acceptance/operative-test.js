...
module('Acceptance | Operative Plan and Operation Report', function(hooks) {
  ...
  test('Plan and report creation', function(assert) {
    return runWithPouchDump('operative', async function() {
      ...
      assert.equal(findAll('a.primary-diagnosis:contains(Broken Arm)').length, 1, 'Primary diagnosis appears');
      assert.equal(findAll('a.secondary-diagnosis:contains(Tennis Elbow)').length, 1, 'Secondary diagnosis appears');
      ...
      assert.equal(findAll('span.primary-diagnosis:contains(Broken Arm)').length, 1, 'Primary diagnosis appears as read only');
      assert.equal(findAll('span.secondary-diagnosis:contains(Tennis Elbow)').length, 1, 'Secondary diagnosis appears as read only');
      ...
      assert.equal(findAll('.procedure-listing td.procedure-description:contains(Delete Me)').length, 0, 'Procedure is properly deleted');
      ...
      ).length, 1, 'Procedure from typeahead gets added to procedure list on save');
      ...
      assert.equal(findAll('a:contains(Current Operative Plan)').length, 1, 'Link to newly created plan appears');
      ...
      assert.equal(findAll('button:contains(Complete Plan)').length, 1, 'Complete Plan button appears');
      ...
      assert.dom('.patient-name .ps-info-data').hasText('Joe Bagadonuts', 'Joe Bagadonuts patient header displays');
      assert.equal(findAll('a.primary-diagnosis:contains(Broken Arm)').length, 1, 'Primary diagnosis appears as editable');
      assert.equal(findAll('a.secondary-diagnosis:contains(Tennis Elbow)').length, 1, 'Secondary diagnosis appears as  editable');
      ...
      assert.equal(findAll(`.procedure-listing td.procedure-description:contains(${PROCEDURE_HIP})`).length, 1, `Procedure ${PROCEDURE_HIP} is copied from operative plan`);
      assert.equal(findAll(
        `.procedure-listing td.procedure-description:contains(${PROCEDURE_FIX_ARM})`
      ).length, 1, `Procedure ${PROCEDURE_FIX_ARM} is copied from operative plan`);
      ...
      await click('button:contains(Return)');
      assert.equal(currentURL(), '/patients/edit/cd572865-dcc0-441e-a2ad-be400dc256da', 'Patient edit URL is correct');
      assert.equal(findAll('a.patient-procedure:contains(fix broken arm)').length, 1, 'Procedure/operative report shows on patient header');
