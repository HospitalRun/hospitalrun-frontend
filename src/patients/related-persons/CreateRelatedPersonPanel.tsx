...
import { Panel, Checkbox, Alert, Button } from '@hospitalrun/components'
...
const CreateRelatedPersonPanel = (props: Props): ReactElement => {
  const { t } = useTranslation()
  const { patient, isEditable, onChange, onSave, onCancel, onTypeChange, error } = props
  ...
  return (
    <div>
      <Panel
        title={t('patient.relatedPersons.newRelatedPersonInformation')}
        color="primary"
        collapsible
      >
        ...
        <div className="row float-right">
          <div className="btn-group btn-group-lg mt-3">
            ...
            <Button className="btn-cancel mr-3" color="danger" onClick={onCancel}>
              {t('actions.cancel')}
            </Button>
          </div>
        </div>
...
export default CreateRelatedPersonPanel
